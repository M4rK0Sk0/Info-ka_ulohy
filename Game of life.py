...
def openfile():
    ...
    if counter < 2500:
        ...
        print("Moc veľký súbor")

def loop():
    if button2.config('text')[-1] == 'Pôjde to':
        recalcul()
        win.after(100, loop)

def life():
    if button2.config('text')[-1] == 'Hýbe sa to':
        button2.config(text='Pôjde to')
        loop()
    else:
        button2.config(text='Hýbe sa to, znovu')
...
button1=tk.Button(win,text="Otvor súbor",command=openfile)
button1.pack(side=tk.RIGHT)

button2=tk.Button(win,text="Snáď to pôjde",command=zivotik)
